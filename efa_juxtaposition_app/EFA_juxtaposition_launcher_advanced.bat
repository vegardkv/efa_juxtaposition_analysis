...
if not exist "%APP_DIR%" (
    echo %RED%✗ ERROR: Application directory does not exist%RESET%
    echo Expected: %APP_DIR%
    echo.
    echo Setup instructions:
    echo 1. Navigate to c:\Appl (create if it doesn't exist)
    echo 2. Run: uv init efa_uv_app
    echo 3. Copy the Python application files to the directory
    echo.
    pause
    exit /b 1
)
echo %GREEN%✓ Application directory exists: %APP_DIR%%RESET%
