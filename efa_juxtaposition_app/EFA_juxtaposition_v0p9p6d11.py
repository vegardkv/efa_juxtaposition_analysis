...
class EFA_juxtaposition(tk.Tk):
    ...
    def setup_qc_plot_tab(self):
        ...
        # Create main container with label
        ttk.Label(self.qc_plot_frame, text="Quality Control Plot", 
                 font=('Arial', 12, 'bold')).pack(pady=10)
    ...
    def reset_application(self):
        ...
        try:
            ...
            self.plot_name.set("Analysis Plot")
            self.width.set(12)
            self.height.set(6)
    ...
    def throw_plot_method(self, fv_df=None, hv_df=None, h_color=None, title='', figsize=(6,12), gridlines=1, linewidth=1, meanthrow=1, showlegend=1, z_select='Z'):
        """
        ...
        if meanthrow == 1:
            #plt.plot(fv_df['length'], throwarray.mean(0),color = 'black', linestyle = '--')
            plt.plot(fv_df['length'], np.nanmean(throwarray, axis=0),color = 'black', linestyle = 'dotted', label='Mean Throw')
            #print(throwarray)
            #print(np.nanmean(throwarray, axis=0))
    ...
    def zone_color_plot_method(self, fv_df=None, hv_df=None, ld_dict=None, h_color=None, z_color=None, title='', figsize=(6,12), fv=1, hv=1, fill=1, gridlines=1, linewidth=1, showlegend=1, z_select='Z', disp_orgpoints=1):
        """
    ...
    def zone_juxtscenario_plot_method(self, fv_df=None, hv_df=None, ld_dict=None, h_color=None, z_color=None, title='', figsize=(6,12), gridlines=1, linewidth=1, fv=1, hv=1, apex=1, apexid=1, showlegend=1, z_select='Z', disp_orgpoints=1):
        """
...
2. Loading Horizon Data
3. Converting to Length/Depth
4. Horizon Shifts
5. Generating Plots
...
8. Session Management"""
        ...
        self._add_help_section(frame, "1. Getting Started", heading_font, body_font,
            """The EFA (Edge Fault Analysis) Juxtaposition tool analyzes geological fault data 
to determine which formations are juxtaposed across a fault surface.

Key Concepts:
• Footwall (FW): The rock mass below the fault plane
• Hangingwall (HW): The rock mass above the fault plane
• Juxtaposition: Where different geological units meet across the fault
• Throw: The vertical displacement across the fault""")
        ...
        self._add_help_image(frame, "help_images/fault_diagram.png", 
                             caption="Figure 1: Fault geometry showing footwall and hangingwall")
        
        # Section 2: Loading Data
        self._add_help_section(frame, "2. Loading Horizon Data", heading_font, body_font,
            """Step 1: Click 'Load Files' in the Data Input tab

Step 2: Select horizon files in supported formats:
• Petrel format (X, Y, Z columns)
• Charisma format
• Text files with XYZ coordinates

Step 3: Files will appear in the listbox
• Each file represents one geological horizon
• Order determines stacking order (top to bottom)

Step 4: Assign colors to horizons
• Click on a horizon in the list
• Click 'Edit Horizon Colors' to change colors
• Colors will be used consistently in all plots""")
        
        # Section 3: Converting
        self._add_help_section(frame, "3. Converting to Length/Depth", heading_font, body_font,
            """Purpose: Transforms XYZ coordinates to a 2D representation along the fault

Step 1: Select Z-axis interpretation
• Z = Depth (default)
• Z = Elevation
• Z = TVD (True Vertical Depth)

Step 2: Click 'Convert to Length/Depth'
• Algorithm calculates fault strike and dip
• Projects 3D points onto 2D fault plane
• Separates footwall and hangingwall data

Results:
• Tab 1: Shows length/depth tables for FW and HW
• Tab 4: QC Plot displays original data points
• Mean strike and dip are calculated automatically""")
        
        # Section 4: Horizon Shifts
        self._add_help_section(frame, "4. Horizon Shifts", heading_font, body_font,
            """Purpose: Adjust horizon positions to test different scenarios

Tab 3: Horizon Shift Table
• Each horizon can be shifted vertically
• Shifts simulate different fault throw scenarios
• Enter values in depth units (m, ft, or ftUS)

To Apply Shifts:
1. Enter shift values in the table
2. Click 'Generate Juxtaposition Plot'
3. Plot updates with shifted positions

Tips:
• Positive values shift upward
• Negative values shift downward
• Use zero to include original horizon
• Shifts are relative to original position
• Use NaN to remove created horizon shift""")
        
        # Horizon shift concept figure
        self._add_help_image(frame, "help_images/horizon_shift_concept.png",
                           caption="Figure 3: Conceptual illustration of horizon shifts applied to footwall and hangingwall")
        
        self._add_help_image(frame, "help_images/horizon_shift_example.png",
                           caption="Figure 4: Example of Horizon Shift Table in the application")
        # Section 5: Plots
        self._add_help_section(frame, "5. Generating Plots", heading_font, body_font,
            """Three Main Plot Types:

Throw Plot (Top Left):
• Shows vertical displacement along fault
• Each line represents one horizon
• Mean throw line (dotted) shows average

Juxtaposition Plot (Bottom Left):
• Displays which units are side-by-side
• Footwall horizons: Solid lines with ▲ markers
• Hangingwall horizons: Dashed lines with ▼ markers
• Colored polygons show juxtaposition zones

Scenario Plot (Right):
• Shows specific interval juxtaposition
• Select horizons to define interval
• Color intensity shows overlap percentage

Plot Controls:
• Width/Height: Adjust plot dimensions
• Line Width: Change line thickness
• Grid Lines: Toggle grid on/off
• Plot Name: Add custom title""")
        
        # Add example juxtaposition plot image
        self._add_help_image(frame, "help_images/example_juxtaposition.png",
                           caption="Figure 2: Example juxtaposition plot showing footwall and hangingwall horizons")
        
        # Section 6: Scenarios
        self._add_help_section(frame, "6. Scenarios and Analysis", heading_font, body_font,
            """Scenario Analysis Tab:

Purpose: Analyze juxtaposition between selected horizons

Steps:
1. Select 'From Horizon' (e.g., reservoir top)
2. Select 'To Horizon' (e.g., reservoir base)
3. Click 'Generate Scenario Plot'

Output:
• Scenario plot shows detailed juxtaposition
• Export button saves data to CSV
• Mouse hover shows exact values
...
• Production optimization""")
        ...
        # Section 7: Exporting
        self._add_help_section(frame, "7. Exporting Results", heading_font, body_font,
            """Copy to Clipboard:
...
• Load Session restores complete state""")
        ...
        self._add_help_section(frame, "8. Session Management", heading_font, body_font,
            """Save Session:
• File menu → Save Session
...
• Export plots at high resolution for publications
...
For additional help, contact: jareh@equinor.com"""
...
def hlegend(hcolor_df,zcolor_df):
    ...
    for index, row in hcolor_df.iterrows():
        ...
        custum_legend.append(Line2D([0], [0], color=row['Color'], lw=1, label = row['Alias'] +' (FW)'))
        custum_legend.append(Line2D([0], [0], color=row['Color'],linestyle='--', lw=1, label = row['Alias'] +' (HW)'))
        #print(index)
        #print(row)
...
def interpolate_throw(fv_df, juxt_df, throwarray):
    int_throw = interpolate.interp1d(fv_df['length'], np.nanmean(throwarray, axis=0))
